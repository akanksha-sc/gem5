...
StreamBuffer::streamRead(PacketPtr pkt) {
    DPRINTF(StreamBuffer, "A read request of size %d was received by this"
            " stream buffer\n", pkt->getSize());
...
StreamBuffer::streamWrite(PacketPtr pkt) {
    DPRINTF(StreamBuffer, "A write request of size %d was received by this"
            " stream buffer\n", pkt->getSize());
...
StreamBuffer::status(PacketPtr pkt, bool readStatus) {
    ...
    if (pkt->isRead()) {
        DPRINTF(StreamBuffer, "The status of the buffer has been read."
