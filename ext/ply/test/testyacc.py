...
class YaccErrorWarningTests(unittest.TestCase):
    ...
    def test_yacc_badprec(self):
        try:
            ...
            result = sys.stderr.getvalue()
            self.assert_(
                check_expected(result, "precedence must be a list or tuple\n")
            )
    ...
    def test_yacc_badtok(self):
        try:
            ...
            result = sys.stderr.getvalue()
            self.assert_(
                check_expected(result, "tokens must be a list or tuple\n")
            )
    ...
    def test_yacc_misplaced(self):
        ...
        result = sys.stderr.getvalue()
        self.assert_(
            check_expected(result, "yacc_misplaced.py:32: Misplaced '|'\n")
        )
