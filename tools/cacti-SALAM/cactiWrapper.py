...
for element in memobjects:
    ...
    with open("results/stdout.txt", "a") as out, open(
        "results/stderr.txt", "a"
    ) as err:
        # cacti has a lower limit of 2048 for size
        if element["Size"] < 2048:
            element["Size"] = 2048
        process = subprocess.Popen(
            ["./cactiStats.sh", str(element["Size"]), str(element["Ports"])],
            stdout=out,
            stderr=err,
        )
        process.wait()
...
results_csv = list()
with open("results/out.csv") as results_file:
