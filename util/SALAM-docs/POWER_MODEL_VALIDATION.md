...
| **5b** | Area-proportional HotSpot floorplan from C++ block areas (stats) | **Done** — `power.{fu,reg,spm}AreaUm2`, lazy build at first thermal step |
...
Use `--salam-floorplan-geometry fixed` to restore the Phase 5 placeholder block sizes (3 mm / 1 mm / 2 mm).

### Area stats and floorplan (Phase 5b)

At accelerator init, `SALAMPowerModel` publishes per-block areas as gem5 stats (µm²):

| Stat | Source |
|------|--------|
| `power.fuAreaUm2` | Static FU synthesis (`calculateStaticArea`) |
| `power.regAreaUm2` | Static register footprint (`reg_total × 32 × bit_register area`) |
| `power.spmAreaUm2` | CACTI UCA area at configured SPM size |
| `power.areasReady` | 1 once all three are published |

HotSpot floorplan blocks are sized proportionally within each accelerator row and built **once** on the first thermal step after areas are ready (accel init runs before the first interval). Per-block temperatures are still sampled every `--thermal-interval-cycles`.

Autogenerated floorplans are written to `m5out/salam_hotspot_floorplan.json` (block geometry, source areas in µm², passthrough step count). Validate with:

```bash
python3 configs/example/gem5_library/salam/check_salam_floorplan.py --m5out m5out/
python3 configs/example/gem5_library/salam/compare_salam_thermal_loop.py \
  --power-trace m5out/power_trace.csv \
  --thermal-trace m5out/thermal_trace.csv \
  --label-prefix board.bfs_clstr.top
```
...
| `salam/check_salam_floorplan.py` | Area stats, autogenerated floorplan JSON, thermal spread |
